...
function renderHistory(){const box=$("orderHistory");if(!box)return;const refs=getHistory();if(!refs.length){box.innerHTML='<div class="history-empty">No orders saved in this browser yet. Create an order and it will appear here.</div>';return}box.innerHTML=refs.map(r=>`<div class="history-item"><div><strong>${escapeHtml(r)}</strong><span>Saved order reference</span></div><a class="history-open" href="/track/${encodeURIComponent(r)}">Open status →</a></div>`).join("")}
function escapeHtml(x){return String(x??"").replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[c]))}
function trackReference(){const r=$("trackReference")?.value.trim();if(!r)return show("Enter an order reference.");saveHistory(r);location.href="/track/"+encodeURIComponent(r)}
...
const I18N={en:{available:"Koin is available",emailPlaceholder:"Enter your email",multipleCurrencies:"Multiple currencies",multipleNetworks:"Multiple networks",transparent:"Clear pricing",tracking:"Track every order",privacyMatters:"Privacy matters"},pt:{available:"Koin está disponível",emailPlaceholder:"Introduza o seu email",multipleCurrencies:"Várias moedas",multipleNetworks:"Várias redes",transparent:"Preços transparentes",tracking:"Acompanhe cada pedido",privacyMatters:"A sua privacidade importa"}};
...
function renderRecommendedOffers(list,data){const sec=$("recommendedOffer"),card=$("recommendedOfferCard"),profile=$("offerProfile");if(!sec||!card)return;profile.innerHTML='<span>'+escapeHtml(data.name||'Visitante')+'</span><span>'+escapeHtml(data.country||'')+'</span><span>'+escapeHtml(data.offerNeed||'')+'</span>';const o=list[0];if(!o){card.innerHTML='<div class="recommended-offer"><div class="offer-topline"><span>KOIN</span><span>'+escapeHtml(data.country||'')+'</span></div><h3>Ainda não há uma oferta compatível.</h3><p>Não encontrámos uma oferta KOIN para este perfil agora. Podes consultar a Community para explorar outras opções.</p><a href="/community" class="primary-button">Abrir Community <span>→</span></a></div>';sec.hidden=false;scrollToOffer();return}card.innerHTML='<div class="recommended-offer"><div class="offer-topline"><span>'+escapeHtml(o.label||'Oferta KOIN')+'</span><span>'+escapeHtml(o.country||'')+'</span></div><h3>'+escapeHtml(o.title)+'</h3><div class="offer-price"><strong>'+Number(o.price).toFixed(4)+'</strong><span>'+escapeHtml(o.currency)+' / USDT</span></div>'+(o.network?'<div class="offer-meta">Rede recomendada: '+escapeHtml(o.network)+'</div>':'')+(o.note?'<p>'+escapeHtml(o.note)+'</p>':'')+'<button type="button" class="primary-button use-offer" data-offer-id="'+escapeHtml(o.id)+'">Usar esta oferta <span>→</span></button></div>';sec.hidden=false;const b=card.querySelector('.use-offer');if(b)b.onclick=()=>applyOffer(o);scrollToOffer()}
