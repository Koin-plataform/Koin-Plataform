...
(function initWelcome(){
 ...
 async function finish(){data.name=(nameInput?.value||data.name||"").trim();if(!data.name)return show("Enter your name.");data.completed=true;data.completedAt=new Date().toISOString();localStorage.setItem("koinWelcome",JSON.stringify(data));await submitOnboarding(data);overlay.classList.remove("show");overlay.setAttribute("aria-hidden","true");setTimeout(()=>overlay.remove(),120);scrollToOffer()}
 function begin(){notice.hidden=true;quiz.hidden=false;render(1);setTimeout(()=>nameInput?.focus(),80)}
 ...
 document.querySelectorAll(".welcome-next").forEach(btn=>btn.addEventListener("click",()=>{data.name=(nameInput?.value||"").trim();if(!data.name)return show("Enter your name.");render(2)}));
 nameInput?.addEventListener("keydown",e=>{if(e.key==='Enter'){e.preventDefault();document.querySelector('.welcome-next')?.click()}});
 (async()=>{if(data.completed){overlay.remove();loadRecommendedOffer(data);return}overlay.classList.add("show");overlay.setAttribute("aria-hidden","false");notice.hidden=false;quiz.hidden=true})();
})();
