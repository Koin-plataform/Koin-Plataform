...
function update(){const c=currency(),n=CONFIG.networks[network?.value]||CONFIG.networks.TRC20,a=Number(amount?.value)||0;usdt&&(usdt.textContent=a.toFixed(2)+" USDT");fee&&(fee.textContent=n.fee.toFixed(2)+" USDT");unit&&(unit.textContent=`1 USDT = ${c.price.toFixed(2)} ${country.value}`);total&&(total.textContent=((a+n.fee)*c.price).toFixed(2)+" "+country.value)}
