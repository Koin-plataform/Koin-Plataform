...
function renderHistory(){const box=$("orderHistory");if(!box)return;const refs=getHistory();if(!refs.length){box.innerHTML='<div class="history-empty">No orders saved in this browser yet. Create an order and it will appear here.</div>';return}box.innerHTML=refs.map(r=>`<div class="history-item"><div><strong>${escapeHtml(r)}</strong><span>Saved order reference</span></div><a class="history-open" href="/status/${encodeURIComponent(r)}">Open status →</a></div>`).join("")}
function escapeHtml(x){return String(x??"").replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[c]))}
function trackReference(){const r=$("trackReference")?.value.trim();if(!r)return show("Enter an order reference.");saveHistory(r);location.href="/status/"+encodeURIComponent(r)}
